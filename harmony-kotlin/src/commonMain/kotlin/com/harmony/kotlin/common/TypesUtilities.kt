...
fun randomByteArray(amount: Int = randomInt(1, ANY_ITEMS_COUNT)): ByteArray = Random.nextBytes(amount)
...
fun randomByteArrayList(): List<ByteArray> {
  val elements = randomInt(0, ANY_ITEMS_COUNT)
...
fun randomPairList() = (0..randomInt(0, ANY_ITEMS_COUNT)).map { Pair(randomString().removeSpaces(), randomString().removeSpaces()) }.toList()
