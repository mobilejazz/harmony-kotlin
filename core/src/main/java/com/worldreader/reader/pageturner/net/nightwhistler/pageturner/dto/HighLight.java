  ...
  public static List<HighLight> fromJSON(String fileName, String jsonSource) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        HighLight highLight = new HighLight(json.getString(Fields.displayText.name()),
            json.getInt(Fields.index.name()), json.getInt(Fields.start.name()),
            json.getInt(Fields.end.name()), json.getInt(Fields.color.name()));
