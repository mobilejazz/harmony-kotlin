 ...
 */
package com.worldreader.reader.pageturner.net.nightwhistler.pageturner.animation;
...
public class PageCurlAnimator implements Animator {
  ...
  @Override public synchronized void advanceOneFrame() {
  ...
  private float debugDrawPoint(Canvas canvas, String name, Vector2D point, int color, float posX,
      float posY) {
    return debugDrawPoint(canvas, name + " " + point.toString(), point.x, point.y, color, posX,
        posY);
  }

  private float debugDrawPoint(Canvas canvas, String name, float X, float Y, int color, float posX,
      float posY) {
   ...
   */
  public static void drawTextShadowed(Canvas canvas, String text, float x, float y, Paint textPain,
      Paint shadowPaint) {
  ...
  public void SetCurlSpeed(int curlSpeed) {
    if (curlSpeed < 1) throw new IllegalArgumentException("curlSpeed must be greated than 0");
  ...
  private Vector2D CapMovement(Vector2D point, boolean bMaintainMoveDir) {
    ...
    if (point.distance(mOrigin) > mFlipRadius) {
      if (bMaintainMoveDir) {
        ...
        point.y = (float) (Math.sin(Math.acos(Math.abs(point.x - mOrigin.x) / mFlipRadius))
            * mFlipRadius);
      }
    }
    return point;
  ...
  @Override public int getAnimationSpeed() {
    return 30;
  }
