...
import com.worldreader.reader.pageturner.net.nightwhistler.pageturner.dto.SearchResult;
...
public class SearchTextTask extends QueueableAsyncTask<String, SearchResult, List<SearchResult>> {
  ...
  public SearchTextTask(Book book, ResourcesLoader resourcesLoader) {
    this.book = book;
  ...
  @Override public Option<List<SearchResult>> doInBackground(String... params) {
    ...
    try {
      ...
      PageTurnerSpine spine = new PageTurnerSpine(book, resourcesLoader);
      ...
      for (int index = 0; index < spine.size(); index++) {
        ...
        if (!isEmpty(currentResource)) {
          ...
          while (matcher.find()) {
            ...
            SearchResult res =
                new SearchResult(searchTerm, text, index, matcher.start(), matcher.end());
