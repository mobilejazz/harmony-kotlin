...
public class PackageDocumentMetadataWriter extends PackageDocumentBase {
  ...
  public static void writeMetaData(Book book, XmlSerializer serializer)
    ...
    writeSimpleMetdataElements(DCTags.description, book.getMetadata().getDescriptions(),
        serializer);
    ...
    if (book.getCoverImage() != null) { // write the cover image
      ...
      serializer.attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.content,
    ...
    serializer.attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.name,
        OPFValues.generator);
    serializer.attribute(EpubWriter.EMPTY_NAMESPACE_PREFIX, OPFAttributes.content,
        Constants.EPUBLIB_GENERATOR_NAME);
    serializer.endTag(NAMESPACE_OPF, OPFTags.meta);

    serializer.endTag(NAMESPACE_OPF, OPFTags.metadata);
  }

  private static void writeSimpleMetdataElements(String tagName, List<String> values,
   ...
   * If no identifier has bookId == true then the first bookId identifier is written as the
   * primary.
   *
   * @throws IOException
   * @throws IllegalStateException
