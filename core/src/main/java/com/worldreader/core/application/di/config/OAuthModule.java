...
import com.worldreader.core.datasource.network.general.retrofit.interceptor.WorldreaderOAuthTokenInterceptor2;
...
@Module public class OAuthModule {
  ...
  @Provides @Singleton
  static WorldreaderOAuthTokenInterceptor2 provideOAuthTokenInterceptorMobileJazz(Logger logger) {
    return new WorldreaderOAuthTokenInterceptor2(logger);
  }
  ...
  @Provides @Singleton
  static OAuthBdDataSource provideOABdDataSource(OAuthBdDataSourceImpl oAuthBdDataSource) {
  ...
  @Provides @Singleton @WorldreaderOAuthClientSecret
  static String provideWorldreaderOAuthClientSecret() {
    return BuildConfig.WORLDREADER_OAUTH_CLIENT_SECRET;
  }

  @Provides @Singleton
  static OAuthApiService2 provideOAuthApiService(WorldreaderUserRetrofitApiManager2 apiManager) {
    return apiManager.oAuthApiService();
  }

  @Provides @Singleton
  static AuthApiService2 provideAuthApiService(WorldreaderUserRetrofitApiManager2 apiManager) {
    return apiManager.authApiService();
  }

  @Provides @Singleton
  static OAuthNetworkDataSource provideOAuthNetworkDataSource(OAuthApiService2 oAuthApiService,
      AuthApiService2 authApiService, @WorldreaderOAuthClientId String clientId,
      @WorldreaderOAuthClientSecret String secret) {
