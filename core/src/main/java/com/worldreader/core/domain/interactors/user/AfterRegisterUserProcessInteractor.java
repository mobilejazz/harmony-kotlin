...
@PerActivity public class AfterRegisterUserProcessInteractor {
  ...
  private final AnonymousUserScoreSynchronizationProcessInteractor
  ...
  @Inject public AfterRegisterUserProcessInteractor(final ListeningExecutorService executor,
      final SaveUserInteractor saveUserInteractor, final GetUserInteractor getUserInteractor,
      final DeleteUserInteractor deleteUserInteractor,
      final GetAllUserBookInteractor getAllUserBookInteractor,
      final GetAllUserMilestonesInteractor getAllUserMilestonesInteractor,
      ...
      final SaveSessionInteractor saveSessionInteractor, final Dates dateUtils) {
    ...
    this.anonymousUserScoreSynchronizationProcessInteractor =
        anonymousUserScoreSynchronizationProcessInteractor;
  ...
  public ListenableFuture<User2> execute(final From from, final User2 user2,
      final List<Integer> categoriesId, final boolean isAnonymous) {
  ...
  @NonNull SafeRunnable getInteractorRunnable(final SettableFuture<User2> future, final From from,
      final User2 user, final List<Integer> categoriesId, final boolean isAnonymous) {
    return new SafeRunnable() {
      @Override protected void safeRun() throws Throwable {
        handleNewUserRegisteredFromEverythingElseProcess(future, user, isAnonymous);
      }

      private void handleNewUserRegisteredFromEverythingElseProcess(
          final SettableFuture<User2> future, final User2 user, final boolean isAnonymous)
          throws Throwable {
        // 0 - Save new user
        saveUserInteractor.execute(user,
            isAnonymous ? SaveUserInteractor.Type.ANONYMOUS : SaveUserInteractor.Type.LOGGED_IN,
            MoreExecutors.directExecutor()).get();

        if (isAnonymous) {
          final List<UserMilestone> milestones =
              createUserMilestonesInteractor.execute(user.getId(), Collections.<Integer>emptyList(),
                  MoreExecutors.directExecutor()).get();
          ...
          putAllUserMilestonesInteractor.execute(putUserMilestonesStorageSpec, milestones,
        ...
        final UserStorageSpecification getAnonymousUserSpec =
            new UserStorageSpecification(UserStorageSpecification.UserTarget.ANONYMOUS);
        final ListenableFuture<User2> getAnonymousUserFuture =
            getUserInteractor.execute(getAnonymousUserSpec, MoreExecutors.directExecutor());
        ...
        try {
          ...
          final ListenableFuture<User2> updateUserGoalsFuture =
              updateUserGoalsInteractor.execute(pagesPerDay, minChildAge, maxChildAge,
                  MoreExecutors.directExecutor());

          updateUserGoalsFuture.get();

          // 4 - Get anonymous userbooks
          final GetAllUserBooksStorageSpec getAllUserBooksStorageSpec =
              new GetAllUserBooksStorageSpec();
          final ListenableFuture<com.google.common.base.Optional<List<UserBook>>>
              getAnonymousUserBooksFuture =
              getAllUserBookInteractor.execute(getAllUserBooksStorageSpec,
                  MoreExecutors.directExecutor());
          final List<UserBook> anonymousUserBooks =
              getAnonymousUserBooksFuture.get().or(Collections.<UserBook>emptyList());

          // 5 - If we have userbooks, then we have to send it
          if (!anonymousUserBooks.isEmpty()) {
            UserBookNetworkSpecification userBookNetworkSpecification =
                new UserBookNetworkSpecification();
            final ListenableFuture<com.google.common.base.Optional<List<UserBook>>>
                putAllUserBooksFuture =
                putAllUserBooksInteractor.execute(userBookNetworkSpecification, anonymousUserBooks,
                    MoreExecutors.directExecutor());
            final List<UserBook> updatedUserBooks =
                putAllUserBooksFuture.get().or(Collections.<UserBook>emptyList());

            // 5.1 - If we have updated userbooks, store it for the current user
            if (!updatedUserBooks.isEmpty()) {
              final UserBookStorageSpecification updatedUserBookStorageSpecification =
                  new PutAllUserBooksStorageSpec();
              putAllUserBooksInteractor.execute(updatedUserBookStorageSpecification,
                  updatedUserBooks, MoreExecutors.directExecutor());
            }
          }

          // 6 - Get anonymous completed milestones
          final GetAllUserMilestoneStorageSpec getAllUserMilestoneStorageSpec =
              new GetAllUserMilestoneStorageSpec();
          final ListenableFuture<com.google.common.base.Optional<List<UserMilestone>>>
              getAllUserMilestonesFuture =
              getAllUserMilestonesInteractor.execute(getAllUserMilestoneStorageSpec,
                  MoreExecutors.directExecutor());
          final List<UserMilestone> anonymousUserMilestones =
              getAllUserMilestonesFuture.get().or(Collections.<UserMilestone>emptyList());
          ...
          if (!anonymousUserMilestones.isEmpty()) {
            final ListenableFuture<List<UserMilestone>> updateMilestonesFuture =
                putAllUserMilestonesNetworkInteractor.execute(anonymousUserMilestones,
                    MoreExecutors.directExecutor());
          ...
          final ListenableFuture<Boolean> anonymousUserScoreSyncronizationFuture =
              anonymousUserScoreSynchronizationProcessInteractor.execute(anonymousUser.getId(),
          ...
          if (favoriteCategories != null && !favoriteCategories.isEmpty()) {
            final List<Integer> categoriesTransformed =
                Lists.newArrayListWithCapacity(favoriteCategories.size());
            for (final String categoryString : favoriteCategories) {
              categoriesTransformed.add(Integer.valueOf(categoryString));
            }
            saveUserCategoriesInteractor.execute(categoriesTransformed,
                MoreExecutors.directExecutor());
          }

          // 10 - Get the new updated user
          final NetworkSpecification userNetworkSpec = new NetworkSpecification();
          final ListenableFuture<User2> getUpdatedUserFuture =
              getUserInteractor.execute(userNetworkSpec, MoreExecutors.directExecutor());
          ...
          final ListenableFuture<List<UserMilestone>> newUserMilestonesFuture =
              createUserMilestonesInteractor.execute(id, rawUpdatedUserMilestones,
                  MoreExecutors.directExecutor());
          ...
          final ListenableFuture<User2> newUpdatedUserFuture =
              saveUserInteractor.execute(newUpdatedUser, SaveUserInteractor.Type.LOGGED_IN,
                  MoreExecutors.directExecutor());
          ...
          final ListenableFuture<com.google.common.base.Optional<List<UserMilestone>>>
              updateUserMilestonesFuture =
              putAllUserMilestonesInteractor.execute(putUserMilestonesStorageSpec,
                  updatedUserMilestones, MoreExecutors.directExecutor());
          updateUserMilestonesFuture.get();

          // 14 - Delete anonymous user
          deleteUserInteractor.execute(DeleteUserInteractor.Type.ANONYMOUS,
              MoreExecutors.directExecutor()).get();
          ...
          if (e.getCause() instanceof UserNotFoundException) {
            ...
            if (categoriesId != null && categoriesId.size() > 0) {
              final User2 userWithCategories =
                  saveUserCategoriesInteractor.execute(categoriesId, MoreExecutors.directExecutor())
                      .get();

              // Update the new user with the category ids.
              saveUserInteractor.execute(userWithCategories, SaveUserInteractor.Type.LOGGED_IN,
                  MoreExecutors.directExecutor());
            }
      ...
      private void saveOnBoardingPassedAndSaveSessionPassed(final SettableFuture<User2> future,
          final User2 user) {
        // 1.- Save that the user has passed the onboarding
        final ListenableFuture<Boolean> saveOnBoardingPassedFuture =
            saveOnBoardingPassedInteractor.execute(true, MoreExecutors.directExecutor());

        // 2.- Save the session
        final ListenableFuture<Boolean> saveSessionFuture =
            Futures.transformAsync(saveOnBoardingPassedFuture,
                new AsyncFunction<Boolean, Boolean>() {
                  @Override public ListenableFuture<Boolean> apply(@Nullable final Boolean input)
                      throws Exception {
