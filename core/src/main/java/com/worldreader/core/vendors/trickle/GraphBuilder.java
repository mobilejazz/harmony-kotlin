...
class GraphBuilder<R> extends ConfigurableGraph<R> {
  ...
  GraphBuilder(String name, TrickleNode<R> node, ImmutableList<Dep<?>> inputs,
      ImmutableList<Graph<?>> predecessors, Optional<AsyncFunction<Throwable, R>> fallback,
      boolean debug) {
  ...
  static <E> ImmutableList<E> with(ImmutableList<E> list, List<E> elements) {
    return ImmutableList.<E>builder().addAll(list).addAll(elements).build();
  }
  ...
  @Override public ConfigurableGraph<R> fallback(AsyncFunction<Throwable, R> handler) {
    return withFallback(handler);
  }

  @Override public ConfigurableGraph<R> named(String name) {
    return withName(name);
  }

  @Override public ConfigurableGraph<R> after(Graph<?>... predecessors) {
    return withPredecessors(copyOf(predecessors));
  }

  @Override public <P> Graph<R> bind(Input<P> input, P value) {
    return new PreparedGraph<R>(this, debug).bind(input, value);
  }

  @Override public <P> Graph<R> bind(Input<P> input, ListenableFuture<P> inputFuture) {
    return new PreparedGraph<R>(this, debug).bind(input, inputFuture);
  }

  @Override public ListenableFuture<R> run() {
    return new PreparedGraph<R>(this, debug).run();
  }

  @Override public ListenableFuture<R> run(Executor executor) {
    return new PreparedGraph<R>(this, debug).run(executor);
  }

  @Override ListenableFuture<R> run(TraverseState state) {
    return new PreparedGraph<R>(this, debug).run(state);
  }

  @Override public Graph<R> debug(boolean debug) {
  ...
  @Override public List<? extends NodeInfo> arguments() {
    return Lists.transform(inputs, new Function<Dep<?>, NodeInfo>() {
      @Override public NodeInfo apply(@Nullable Dep<?> input) {
  ...
  @Override public Iterable<? extends NodeInfo> predecessors() {
    return predecessors;
  }
  ...
  static final class GraphBuilder2<A, B, R> extends GraphBuilder<R>
    ...
    @Override public ConfigurableGraph<R> with(Parameter<A> arg1, Parameter<B> arg2) {
  ...
  static final class GraphBuilder5<A, B, C, D, E, R> extends GraphBuilder<R>
    ...
    @Override
    public ConfigurableGraph<R> with(Parameter<A> arg1, Parameter<B> arg2, Parameter<C> arg3,
        Parameter<D> arg4, Parameter<E> arg5) {
  ...
  static final class ListGraphBuilder<A, R> extends GraphBuilder<R>
    ...
    @Override public ConfigurableGraph<R> with(List<? extends Parameter<A>> args) {
