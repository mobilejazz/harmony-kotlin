...
import static com.worldreader.core.vendors.trickle.GraphExceptionWrapper.wrapException;
...
class NodeExecutionFallback<R> implements AsyncFunction<Throwable, R> {
  ...
  public NodeExecutionFallback(GraphBuilder<R> graph,
      TraverseState.FutureCallInformation currentCall, TraverseState state) {
  ...
  @Override public ListenableFuture<R> apply(@Nullable Throwable t) throws Exception {
    if (graph.getFallback().isPresent()) {
      try {
        return graph.getFallback().get().apply(t);
      } catch (Exception e) {
