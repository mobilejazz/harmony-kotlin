...
final class PreparedGraph<R> extends Graph<R> {
  ...
  private ListenableFuture<R> future(final TraverseState state) {
    ...
    ListenableFuture<R> combine = nodeFuture(futures, allFuture, state.getExecutor());

    return Futures.catchingAsync(combine, Throwable.class,
  ...
  private ListenableFuture<R> nodeFuture(final ImmutableList<ListenableFuture<?>> values,
    ...
    return Futures.transformAsync(doneSignal, new AsyncFunction<List<Object>, R>() {
      @Override public ListenableFuture<R> apply(List<Object> input) {
        ...
        return graph.getNode()
            .run(Lists.transform(values, new Function<ListenableFuture<?>, Object>() {
              @Override public Object apply(ListenableFuture<?> input) {
  ...
  private PreparedGraph<R> addToInputs(Input<?> input, Object value) {
    ...
    return new PreparedGraph<R>(graph,
        ImmutableMap.<Input<?>, Object>builder().putAll(inputBindings).put(input, value).build(),
        debug);
  }
