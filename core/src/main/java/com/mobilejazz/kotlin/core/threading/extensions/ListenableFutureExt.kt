...
// Creation
inline fun <A> Future(executor: ExecutorService = DirectExecutor, crossinline block: () -> A): Future<A> {
...
fun emptyListenableFuture(): Future<Void> = Futures.immediateFuture(null)

fun <A> Throwable.toListenableFuture(): Future<A> = Futures.immediateFailedFuture(this)

// Monadic Operations
inline fun <A, B> Future<A>.map(executor: Executor = DirectExecutor, crossinline f: (A) -> B): Future<B> =
        Futures.transform(this, com.google.common.base.Function { f(it!!) }, executor)

inline fun <A, B> Future<A>.flatMap(executor: Executor = DirectExecutor, crossinline f: (A) -> Future<B>): Future<B> =
        Futures.transformAsync(this, AsyncFunction { f(it!!) }, executor)

fun <A> Future<Future<A>>.flatten(): Future<A> = flatMap { it }

inline fun <A> Future<A>.filter(executor: Executor = DirectExecutor, crossinline predicate: (A) -> Boolean): Future<A> =
        map(executor) {
            if (predicate(it)) it else throw NoSuchElementException("Future.filter predicate is not satisfied")
        }

fun <A, B> Future<A>.zip(other: Future<B>, executor: Executor = DirectExecutor): Future<Pair<A, B>> =
        zip(other, executor) { a, b -> a to b }

inline fun <A, B, C> Future<A>.zip(other: Future<B>, executor: Executor = DirectExecutor, crossinline f: (A, B) -> C): Future<C> =
...
inline fun <A> Future<A>.recoverWith(executor: Executor = DirectExecutor, crossinline f: (Throwable) -> Future<A>): Future<A> {
...
inline fun <A> Future<A>.fallbackTo(executor: Executor = DirectExecutor, crossinline f: () -> Future<A>): Future<A> =
        recoverWith(executor, { f() })

// Callbacks
inline fun <A> Future<A>.onFailure(executor: Executor = DirectExecutor, crossinline f: (Throwable) -> Unit): Future<A> {
...
inline fun <A> Future<A>.onSuccess(executor: Executor = DirectExecutor, crossinline f: (A) -> Unit): Future<A> {
    Futures.addCallback(this, object : FutureCallback<A> {
        override fun onSuccess(result: A?) {
            f(result!!)
        }
...
inline fun <A> Future<A>.onComplete(executor: Executor = DirectExecutor, crossinline onFailure: (Throwable) -> Unit, crossinline onSuccess: (A) -> Unit): Future<A> {
    Futures.addCallback(this, object : FutureCallback<A> {
        override fun onSuccess(result: A?) {
            onSuccess(result!!)
        }
...
object FutureObject {
    ...
    fun <A> allAsList(futures: Iterable<Future<A>>, executor: Executor = DirectExecutor): Future<List<A>> =
            futures.fold(mutableListOf<A>().toListenableFuture()) { fr, fa ->
                fr.zip(fa, executor) { r, a -> r.add(a); r }
            }.map(executor) { it.toList() }

    fun <A> successfulList(futures: Iterable<Future<A>>, executor: Executor = DirectExecutor): Future<List<A>> =
            futures.fold(mutableListOf<A>().toListenableFuture()) { fr, fa ->
    ...
    fun <A, R> fold(futures: Iterable<Future<A>>, initial: R, executor: Executor = DirectExecutor, op: (R, A) -> R): Future<R> =
            fold(futures.iterator(), initial, executor, op)

    fun <A, R> fold(iterator: Iterator<Future<A>>, initial: R, executor: Executor = DirectExecutor, op: (R, A) -> R): Future<R> =
    ...
    fun <A> reduce(iterable: Iterable<Future<A>>, executor: Executor = DirectExecutor, op: (A, A) -> A): Future<A> {
    ...
    fun <A, B> transform(iterable: Iterable<Future<A>>, executor: Executor = DirectExecutor, f: (A) -> B): Future<List<B>> =
